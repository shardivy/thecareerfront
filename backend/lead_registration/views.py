...
from accounts.utils import generate_otp, generate_password, generate_role_id, send_credentials_email
...
from lead_registration.serializers import AddUserSerializer, HobbySerializer, LeadSerializer, StreamSerializer, StudentAcademicHistorySerializer, StudentHobbySerializer, StudentStreamSerializer, StudentSubjectPreferenceSerializer, SubjectSerializer, UserProgramPackageResponseSerializer
            ...
            return Response(
                {
                    ...
                    "errors": (
                        ["Phone number already exists."] if "phone" in error_message.lower()
                        else ["Email already exists."] if "email" in error_message.lower()
                        else []
                    )
                },
                status=status.HTTP_400_BAD_REQUEST
...
#                     phone=lead.phone,
#                     role=student_role,
#                     password=make_password("Temp@123"),
#                     is_active=True
#                 )

#                 # 2️⃣ Create StudentProfile (ONLY valid fields)
...
#         except Exception as e:
#             return Response(
#                 {"message": "Conversion failed", "error": str(e)},
#                 status=status.HTTP_500_INTERNAL_SERVER_ERROR
#             )
